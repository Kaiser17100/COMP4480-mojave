...
def upload_hss_fences(connection, hss_list):
    if not hss_list:
        return
    print(f"Uploading {len(hss_list)} HSS Circular Fences to MAVLink Map...")
    for i, hss in enumerate(hss_list):
        lat = int(hss.get("hssEnlem", 0.0) * 1e7)
        lon = int(hss.get("hssBoylam", 0.0) * 1e7)
        radius = float(hss.get("hssYaricap", 0.0))
...
def main_loop():
    ...
    while True:
        ...
        if now - last_hss_fetch >= 60.0:
            try:
                hss_data = telemetry.get_hss(session, BASE_URL)
                if hss_data:
                    hss_list = hss_data
                    upload_hss_fences(connection, hss_list)
            except Exception as e:
