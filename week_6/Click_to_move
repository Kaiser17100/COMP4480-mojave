...
import os   # Added for Gazebo camera enable
...
def run():
    ...
    while True:
        ...
        # --- Process Camera Clicks ---
        click_data = cmd.consume_click()
        if click_data and not override:
            x_norm, y_norm = click_data
            # Max turn per click: 45 degrees yaw, 25 degrees pitch
            new_yaw = (current_yaw + (x_norm * 45.0) + 180) % 360 - 180
            new_pitch = max(-30.0, min(30.0, y_norm * 25.0))
            
            # Clear old targets so pitch/yaw take precedence
            cmd.update('roll', None)
            cmd.update('yaw', new_yaw)
            cmd.update('alt', None)
            cmd.update('pitch', new_pitch)
            print(f"\n[Camera] Clicked! Turning Nose to Yaw: {new_yaw:.1f}°, Pitch: {new_pitch:.1f}°\ncmd> ", end="")
